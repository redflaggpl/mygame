...
class AuthUser extends Utils {
	...
	/**
	 * @desc destruye variables de sesión
	 * @param void
	 * @return void
	 */
	function logout() {
	...
	function isLoggedIn(){
		if(!isset($_SESSION['login']) || !isset($_SESSION['passwd']) || !isset($_SESSION['uid'])){
			...
			$this->setTiempoSesion();
			return true;
		}
	}
	
	function setTiempoSesion()
	{
		$fechaGuardada = $_SESSION['ultimoAcceso'];
		//Obtenemos la fecha actual
		$ahora = date("Y-n-j H:i:s");
		$tiempoTranscurrido = (strtotime($ahora)-strtotime($fechaGuardada));
		//Substituimos x por el tiempo que queramos (en segundos)
		if($tiempoTranscurrido >= 360) {
			session_destroy();
			//Volvemos a la página principal o a la del login
